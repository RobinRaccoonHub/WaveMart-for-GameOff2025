...
func find_path_to_target(guest_spawner: Node2D, target_node: Node2D) -> PackedVector2Array:
	var target_position := local_to_map(target_node.global_position)
	var start_position := local_to_map(guest_spawner.global_position)
...
func _prepare_pathfinding_graph() -> void:
	for current_cell: Vector2i in floor_tiles:
